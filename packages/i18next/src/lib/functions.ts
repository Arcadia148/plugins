...
import type { StringMap, TFunction, TFunctionKeys, TFunctionResult, TOptions } from 'i18next';
import type {
...
const getLocales = lazy(() => {
	const locales = new Map<LocaleString, TFunction>();
	for (const [locale, t] of container.i18n.languages) {
		const resolvedLocale = resolveLocale(locale);
		if (!isSupportedDiscordLocale(resolvedLocale)) {
		...
		locales.set(resolvedLocale, t);
	}
	return locales;
});
const getDefaultT = lazy(() => {
	const defaultLocale = resolveLocale(container.i18n.options.defaultName ?? 'en-US');
...
function resolveLocale(localeKey: string): LocaleString | string {
	switch (localeKey) {
		case 'bg-BG':
